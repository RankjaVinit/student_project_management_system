...
import AdminGroupActions from "./actions";
...
export default async function AdminGroupDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="space-y-6">
            <div>
                <Link href="/admin/projects" className="text-sm text-indigo-600 hover:text-indigo-900 mb-4 inline-block">
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
                    ...
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        group.approval_status === 'APPROVED' ? 'bg-green-100 text-green-800' :
                ...
                <div className="border-t border-gray-200">
                    <dl>
                        ...
                        <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt className="text-sm font-medium text-gray-500">Guide Faculty</dt>
                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        ...
                        <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {group.created_at ? new Date(group.created_at).toLocaleString() : "N/A"}
                            </dd>
                        </div>
                        <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt className="text-sm font-medium text-gray-500">Admin Actions</dt>
                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <AdminGroupActions groupId={group.project_group_id} currentStatus={group.approval_status} />
                            </dd>
                        </div>
                    </dl>
                </div>
                        ...
                        <ul className="divide-y divide-gray-200">
                            {meetings.map(meeting => (
                                <li key={meeting.project_meeting_id} className="px-4 py-4 sm:px-6">
                                    ...
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-900 font-medium">Purpose:</p>
                                        <p className="text-sm text-gray-500">{meeting.meeting_purpose || "No purpose specified"}</p>
                                    </div>
            ...
            <div className="bg-white shadow sm:rounded-lg">
                <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">Documents ({documents.length})</h3>
                </div>
                <div className="border-t border-gray-200">
                    {documents.length === 0 ? (
